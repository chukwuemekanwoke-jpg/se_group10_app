...
document.addEventListener("DOMContentLoaded", () => {
    ...
    predictBtn.addEventListener("click", async () => {
        ...
        predictionResult.textContent =
            `Preparing prediction for ${window.selectedStation.name} at ${dateValue} ${timeValue}...`;

        try {
            const stationId = window.selectedStation.number;

            const resp = await fetch(
                `/api/predict?station_id=${stationId}&date=${dateValue}&time=${timeValue}`
            );
            ...
            <strong>Station:</strong> ${data.station_name}<br>
            <strong>Date:</strong> ${data.date}<br>
            <strong>Time:</strong> ${data.time}<br>
            <strong>Predicted available bikes:</strong> ${data.predicted_bikes}
            `;
        } catch (err) {
            console.error(err);
